...
<template>
  <div
    class="h-[calc(100dvh-3.5rem-2.5rem)] md:h-[calc(100vh-5.2rem)] flex flex-col justify-between max-w-5xl mx-auto text-text-primary"
  >
    ...
    >
      ...
      <div class="text-center space-y-4 mb-8 select-none">
        <h1
          class="text-2xl md:text-3xl lg:text-4xl font-brand font-black tracking-tight leading-none text-text-primary"
        >
          SmartLogix
        </h1>
      </div>
      ...
      <div
        class="w-full glass-card rounded-2xl p-3 shadow-lg border border-brand-border bg-card-bg"
      >
      ...
      <div class="flex flex-col gap-3 w-full mt-6">
        ...
        >
          <div class="space-y-1">
            <h3
              class="text-sm font-extrabold text-text-primary tracking-wide line-clamp-1 group-hover:text-brand-accent transition-colors"
            >
              {{ sug.title }}
            </h3>
    ...
    <div v-else class="max-w-3xl mx-auto flex-1 flex flex-col min-h-0 bg-transparent py-4 relative">
      ...
      <div ref="chatContainer" class="flex-grow overflow-y-auto space-y-6 mb-16 scroll-smooth">
        <ChatMessage v-for="msg in messages" :key="msg.id" :message="msg" />

        <!-- Loading streaming indicator -->
        <div
          v-if="loading"
          class="flex items-center gap-3 text-text-secondary text-sm pl-2 select-none animate-pulse"
        >
      ...
      <div class="absolute bottom-0 left-0 right-0 p-3 pb-safe bg-brand-bg/85 backdrop-blur-md">
        <div
          class="glass-card rounded-2xl p-2.5 shadow-lg border border-brand-border bg-card-bg max-w-3xl mx-auto"
        >
